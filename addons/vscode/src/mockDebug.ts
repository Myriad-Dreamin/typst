...
export class TypstDebugSession extends LoggingDebugSession {
  ...
  public constructor(fileAccessor: FileAccessor) {
    ...
    this._runtime.on("stopOnEntry", () => {
      this.sendEvent(new StoppedEvent("entry", TypstDebugSession.threadID));
    });
    ...
    this._runtime.on("stopOnBreakpoint", () => {
      this.sendEvent(
        new StoppedEvent("breakpoint", TypstDebugSession.threadID)
      );
    });
    this._runtime.on("stopOnDataBreakpoint", () => {
      this.sendEvent(
        new StoppedEvent("data breakpoint", TypstDebugSession.threadID)
      );
    });
    this._runtime.on("stopOnInstructionBreakpoint", () => {
      this.sendEvent(
        new StoppedEvent("instruction breakpoint", TypstDebugSession.threadID)
      );
    });
    this._runtime.on("stopOnException", (exception) => {
      if (exception) {
        ...
        this.sendEvent(
          new StoppedEvent("exception", TypstDebugSession.threadID)
        );
      }
    });
  ...
  protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
    ...
    response.body = {
      threads: [
        new Thread(TypstDebugSession.threadID, "thread 1"),
        new Thread(TypstDebugSession.threadID + 1, "thread 2"),
      ],
    };
    this.sendResponse(response);
  }
