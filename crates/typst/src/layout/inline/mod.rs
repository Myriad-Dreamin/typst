...
use crate::introspection::{Introspector, Locator, Meta, MetaElem};
use crate::layout::{
...
fn linebreak_simple<'a>(
    ...
    breakpoints(p, |end, breakpoint, eaten| {
        ...
        let mut attempt = line(engine, p, start..end, breakpoint, eaten);
        ...
        if !width.fits(attempt.width) {
            if let Some((last_attempt, last_end)) = last.take() {
                ...
                start = last_end;
                attempt = line(engine, p, start..end, breakpoint, eaten);
            }
        }
...
fn linebreak_optimized<'a>(
    ...
    let mut table = vec![Entry {
        pred: 0,
        total: 0.0,
        line: line(engine, p, 0..0, Breakpoint::Mandatory, '\0'),
    }];
    ...
    breakpoints(p, |end, breakpoint, eaten| {
        ...
        for (i, pred) in table.iter().enumerate().skip(active) {
            ...
            let attempt = line(engine, p, start..end, breakpoint, eaten);
