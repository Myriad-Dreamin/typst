...
fn items() -> LangItems {
    ...
    //     smart_quote: |double|
    // text::SmartquoteElem::new().with_double(double).pack(),     parbreak:
    // || layout::ParbreakElem::new().pack(),     strong: |body|
    // text::StrongElem::new(body).pack(),     emph: |body|
    // text::EmphElem::new(body).pack(),     raw: |text, lang, block| {
    ...
    //     heading: |level, title|
    // meta::HeadingElem::new(title).with_level(level).pack(),
    ...
    //         elem.pack()
    //     },
    //     term_item: |term, description| layout::TermItem::new(term,
    // description).pack(),     equation: |body, block|
    // math::EquationElem::new(body).with_block(block).pack(),
    //     math_align_point: || math::AlignPointElem::new().pack(),
    //     math_delimited: |open, body, close| math::LrElem::new(open + body +
    // close).pack(),     math_attach: |base, t, b, tl, bl, tr, br| {
